...
class Camera:
    ...
    def __init__(self, spi_bus, cs, skip_sleep=False, debug_text_enabled=False):
        ...
        self.current_resolution_setting = self.RESOLUTION_320X240  # ArduCam driver defines this as mode
        ...
        if self.camera_idx == '3MP':
            utime.sleep_ms(
                self.WHITE_BALANCE_WAIT_TIME_MS)  # fifo length is sometimes a broken value (5 to 8 megabytes) and causes program to fail this helps
            self._wait_idle()
    ...
    def capture_jpg(self):
        if (utime.ticks_diff(utime.ticks_ms(),
                             self.start_time) <= self.WHITE_BALANCE_WAIT_TIME_MS) and self.camera_idx == '5MP':
            ...
            if (self.old_pixel_format != self.current_pixel_format) or self.run_start_up_config:
                ...
                self._wait_idle()
            if self.debug_text_enabled: print('Old_resolution: ', self.old_resolution, 'New_resolution: ',
                                              self.current_resolution_setting)
    ...
    def save_JPG(self, filename="image.jpg",
                 progress_bar=True):  # From the amazing - @chrisrothwell1 - https://github.com/CoreElectronics/CE-Arducam-MicroPython/issues/9
    ...
    def get_JPG_bytes(self):
        """
        Captures and returns the image as a byte array.

        Returns:
            bytes: The image data as a byte array, or None if an error occurs.
        """
        ...
        while recv_len > 0:
            # self._update_progress(progress)  # Removed progress bar updates
            self.spi_bus.readinto(self.image_buffer)
            image_buffer.extend(self.image_buffer)
            recv_len -= self.BUFFER_MAX_LENGTH

            # Search for the end of the JPEG
            inx = self.image_buffer.find(b'\xff\xd9')
            if inx >= 0:
                return bytes(image_buffer[:inx + 2])
            elif len(self.image_buffer) > 0 and self.image_buffer[-1] == 0xff and self.image_buffer[0] == 0xd9:
                return bytes(image_buffer + b'\xd9')
            # else:
            #    image_buffer.extend(self.image_buffer)  # No need to extend here, already done in the readinto

        return None  # Return None if the image is not fully received or an error occurs
    ...
    def resolution(self, new_resolution):
        ...
        if self.camera_idx == '3MP':
            if input_string_lower in self.valid_3mp_resolutions:
                ...
                raise ValueError("Invalid resolution provided for {}, please select from {}".format(self.camera_idx,
                                                                                                    list(
                                                                                                        self.valid_3mp_resolutions.keys())))
            ...
            if input_string_lower in self.valid_5mp_resolutions:
                ...
                raise ValueError("Invalid resolution provided for {}, please select from {}".format(self.camera_idx,
                                                                                                    list(
                                                                                                        self.valid_5mp_resolutions.keys())))
    ...
    def _set_capture(self):
        ...
        while (int(self._get_bit(self.ARDUCHIP_TRIG, self.CAP_DONE_MASK)) == 0):
            if self.debug_text_enabled: print("ARDUCHIP_TRIG register, CAP_DONE_MASK:",
                                              self._get_bit(self.ARDUCHIP_TRIG, self.CAP_DONE_MASK))
            sleep_ms(30)
